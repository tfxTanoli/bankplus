...
**Outcome: not sufficient.** The deploy of `12fa1fb` failed at the same step. The lockfile
fix is kept because the drift was a real defect.

### Second step: remove the `installCommand` override (delete `vercel.json`)
The failing build log contains a line absent from the 09-10 logs:
`Detected bun.lock generated by bun@1.4.x`. Vercel now recognises the Bun 1.4 lockfile
format itself. The override `bunx bun@1.4.2 install` existed only because Vercel's Bun
1.3.14 could not read that format, and it is the one step failing. `vercel.json` held
nothing else, so it was deleted. That restores the pre-`0840098` configuration, whose
default install is known to build successfully.
